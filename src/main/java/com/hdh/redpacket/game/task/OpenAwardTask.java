...
@Configuration
@EnableScheduling
public class OpenAwardTask implements SchedulingConfigurer {
    ...
    public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
        taskRegistrar.addTriggerTask(
                //1.添加任务内容(Runnable)
                () -> gameService.openAward(),
                //2.设置执行周期(Trigger)
                triggerContext -> {
                    //2.1 从数据库获取执行周期
                    TaskCron taskCron = taskCronService.getByType(GameStatusTypeEnum.OPEN_AWARD.getCode());
